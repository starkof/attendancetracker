...
*/
public abstract class FacultyDatabaseObjectAbstract extends MithraAbstractTransactionalDatabaseObject implements MithraTransactionalDatabaseObject, MithraObjectFactory
{
	private SourcelessConnectionManager connectionManager;
	private SchemaManager schemaManager;
	private TablePartitionManager tablePartitionManager;
	private static final String COL_LIST_WITHOUT_PK = "DEPARTMENT_ID,COURSE_ID,PERSON_ID";
	private static final String COL_LIST_WITHOUT_PK_WITH_ALIAS = "t0.DEPARTMENT_ID,t0.COURSE_ID,t0.PERSON_ID";
	private static final String PK_WITH_ALIAS = "t0.FACULTY_ID = ?";
	private static final String PK_INDEX_COLS = "FACULTY_ID";
	protected FacultyDatabaseObjectAbstract()
	{
		super("Faculty", "com.mithraobjects.FacultyFinder",
			4, 4,
	...
	{
		return FacultyFinder.getFinderInstance();
	}

	public static FacultyData allocateOnHeapData()
	{
		return new FacultyData();
	}

	public static FacultyData allocateOffHeapData()
	{
	...
	{
		MithraDataObject data = new FacultyData();
	...
	public Cache instantiateFullCache(MithraConfigurationManager.Config config)
	{
		...
		{
			result = new FullNonDatedTransactionalCache(FacultyFinder.getPrimaryKeyAttributes(), this, FacultyFinder.getImmutableAttributes());
		}
		else
		{
			result = new FullNonDatedCache(FacultyFinder.getPrimaryKeyAttributes(), this, FacultyFinder.getImmutableAttributes(), new NonTransactionalUnderlyingObjectGetter());
		}
	...
	public Cache instantiatePartialCache(MithraConfigurationManager.Config config)
	{
		...
		if (config.isParticipatingInTx())
		{
			result = new PartialNonDatedTransactionalCache(FacultyFinder.getPrimaryKeyAttributes(), this, FacultyFinder.getImmutableAttributes(), config.getCacheTimeToLive(), config.getRelationshipCacheTimeToLive());
		}
		else
		{
			result = new PartialNonDatedCache(FacultyFinder.getPrimaryKeyAttributes(), this, FacultyFinder.getImmutableAttributes(), new NonTransactionalUnderlyingObjectGetter(), config.getCacheTimeToLive(), config.getRelationshipCacheTimeToLive());
		}
	...
	private void initPortal(Cache cache, MithraConfigurationManager.Config config)
	{
		...
		{
			FacultyFinder.initializeClientPortal(this, cache, config);
		}
		else
		{
			FacultyFinder.initializePortal(this, cache, config);
		}

		if (config.isParticipatingInTx())
		{
			Faculty.zConfigFullTx();
		}
		else
		{
			Faculty.zConfigNonTx();
		}
	}
	...
	{
		return getFacultyTableName();
	}

	public String getFacultyTableName() throws MithraDatabaseException
	{
	...
	{
		FacultyData data = (FacultyData)dataObj;
		TimeZone conversionTimeZone = null;
		stm.setInt(pos++, data.getFacultyId());
	}
	...
	public String getColumnListWithPk(String databaseAlias)
	{
		...
		{
			return "t0.FACULTY_ID,t0.DEPARTMENT_ID,t0.COURSE_ID,t0.PERSON_ID";
		}

		StringBuffer result = new StringBuffer((databaseAlias.length()+15)*4);
		result.append(databaseAlias).append(".").append("FACULTY_ID");
		result.append(",").append(databaseAlias).append(".").append("DEPARTMENT_ID");
	...
	public FacultyData inflateFacultyData(ResultSet rs, DatabaseType dt)
	throws SQLException
	{
		FacultyData data = inflateFacultyPkData(rs, dt);
		inflateNonPkFacultyData(2, data, rs, dt);
		return data;
	}

	public FacultyData inflateFacultyPkData(ResultSet _rs, DatabaseType _dt)
	throws SQLException
	{
		FacultyData _data = new FacultyData();
		int _pos = 1;
		_data.setFacultyId(_rs.getInt(_pos++));
		checkNullPrimitive(_rs, _data, "facultyId");
		return _data;
	}

	public void inflateNonPkFacultyData(int _pos, FacultyData _datax, ResultSet _rs, DatabaseType _dt)
	throws SQLException
	{
		{
			FacultyData _data = _datax;
			_data.setDepartmentId(_rs.getInt(_pos++));
			if (_rs.wasNull())
			{
				_data.setDepartmentIdNull();
			}
	...
	{
		return "FACULTY_ID,DEPARTMENT_ID,COURSE_ID,PERSON_ID";
	}
	...
	{
		Faculty newObject = new Faculty();
		newObject.zSetFromFacultyData((FacultyData) newData);
		return newObject;
	}
