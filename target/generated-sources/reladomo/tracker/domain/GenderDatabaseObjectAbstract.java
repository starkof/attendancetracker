...
*/
public abstract class GenderDatabaseObjectAbstract extends MithraAbstractTransactionalDatabaseObject implements MithraTransactionalDatabaseObject, MithraObjectFactory
{
	private SourcelessConnectionManager connectionManager;
	private SchemaManager schemaManager;
	private TablePartitionManager tablePartitionManager;
	private static final String COL_LIST_WITHOUT_PK = "GENDER_NAME";
	private static final String COL_LIST_WITHOUT_PK_WITH_ALIAS = "t0.GENDER_NAME";
	private static final String PK_WITH_ALIAS = "t0.GENDER_ID = ?";
	private static final String PK_INDEX_COLS = "GENDER_ID";
	protected GenderDatabaseObjectAbstract()
	{
		super("Gender", "tracker.domain.GenderFinder",
			2, 2,
	...
	{
		return GenderFinder.getFinderInstance();
	}

	public static GenderData allocateOnHeapData()
	{
		return new GenderData();
	}

	public static GenderData allocateOffHeapData()
	{
	...
	{
		MithraDataObject data = new GenderData();
	...
	public Cache instantiateFullCache(MithraConfigurationManager.Config config)
	{
		...
		{
			result = new FullNonDatedTransactionalCache(GenderFinder.getPrimaryKeyAttributes(), this, GenderFinder.getImmutableAttributes());
		}
		else
		{
			result = new FullNonDatedCache(GenderFinder.getPrimaryKeyAttributes(), this, GenderFinder.getImmutableAttributes(), new NonTransactionalUnderlyingObjectGetter());
		}
	...
	public Cache instantiatePartialCache(MithraConfigurationManager.Config config)
	{
		...
		if (config.isParticipatingInTx())
		{
			result = new PartialNonDatedTransactionalCache(GenderFinder.getPrimaryKeyAttributes(), this, GenderFinder.getImmutableAttributes(), config.getCacheTimeToLive(), config.getRelationshipCacheTimeToLive());
		}
		else
		{
			result = new PartialNonDatedCache(GenderFinder.getPrimaryKeyAttributes(), this, GenderFinder.getImmutableAttributes(), new NonTransactionalUnderlyingObjectGetter(), config.getCacheTimeToLive(), config.getRelationshipCacheTimeToLive());
		}
	...
	private void initPortal(Cache cache, MithraConfigurationManager.Config config)
	{
		...
		{
			GenderFinder.initializeClientPortal(this, cache, config);
		}
		else
		{
			GenderFinder.initializePortal(this, cache, config);
		}

		if (config.isParticipatingInTx())
		{
			Gender.zConfigFullTx();
		}
		else
		{
			Gender.zConfigNonTx();
	...
	{
		return getGenderTableName();
	}

	public String getGenderTableName() throws MithraDatabaseException
	{
	...
	{
		GenderData data = (GenderData)dataObj;
	...
	public String getColumnListWithPk(String databaseAlias)
	{
		...
		{
			return "t0.GENDER_ID,t0.GENDER_NAME";
		}

		StringBuffer result = new StringBuffer((databaseAlias.length()+15)*2);
		result.append(databaseAlias).append(".").append("GENDER_ID");
		result.append(",").append(databaseAlias).append(".").append("GENDER_NAME");
	...
	public GenderData inflateGenderData(ResultSet rs, DatabaseType dt)
	throws SQLException
	{
		GenderData data = inflateGenderPkData(rs, dt);
		inflateNonPkGenderData(2, data, rs, dt);
		return data;
	}

	public GenderData inflateGenderPkData(ResultSet _rs, DatabaseType _dt)
	throws SQLException
	{
		GenderData _data = new GenderData();
		int _pos = 1;
		_data.setGenderId(_rs.getInt(_pos++));
		checkNullPrimitive(_rs, _data, "genderId");
		return _data;
	}

	public void inflateNonPkGenderData(int _pos, GenderData _datax, ResultSet _rs, DatabaseType _dt)
	throws SQLException
	{
		{
			GenderData _data = _datax;
			_data.setGenderName(trimString(_rs.getString(_pos++)));
		}
	}
		...
		TimeZone databaseTimeZone, int pos, DatabaseType dt) throws SQLException
	{
		...
		TimeZone conversionTimeZone = null;
		stm.setInt(pos++, data.getGenderId());
		if (data.isGenderNameNull())
		{
			stm.setNull(pos++, java.sql.Types.VARCHAR);
		}
		else
		{
			stm.setString(pos++, data.getGenderName());
		}
	}

	public String getInsertFields()
	{
		return "GENDER_ID,GENDER_NAME";
	}
	...
	{
		Gender newObject = new Gender();
		newObject.zSetFromGenderData((GenderData) newData);
		return newObject;
	}
